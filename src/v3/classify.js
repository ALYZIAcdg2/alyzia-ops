const REPORTS = [
  ...
  [/\b(?:FQTV|FQA|FQT-|PDF-AFQTA|PDF-IPPS)\b/i,'FQTV'],[/\b(?:INF|INFANT|INFT|PDF-(?:INF|CINFT))\b/i,'INF'],
  [/\b(?:ETKT|ETICKET|TICKET)\b/i,'ETKT'],[/\bEMD\b/i,'EMD'],
  [/\b(?:INBOUND|\bINC\b)\b/i,'INBOUND'],[/\b(?:ONCARRIAGE|OUTBOUND|\bONC\b)\b/i,'OUTBOUND'],
  [/\b(?:SSR|WCHR|WCHS|WCHC|UMNR|PRM|MAAS)\b|\bPDF-(?:OSPL|PSPL)/i,'SSR'],
...
export function classifyDocument(document) {
  ...
  const title = head.match(/\bLIST\s+OF\s*:\s*([^\n\r]{2,120})/i)?.[1]?.replace(/\b(?:TOTAL|TTL)\b.*$/i,'').replace(/\b[FJCWSYM]\s*\d+(?=\s|$)/gi,'').trim() ||
