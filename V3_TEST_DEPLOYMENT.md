...
On the `feature/import-v3-staging` branch, `.github/workflows/deploy-v3-test.yml` can provision and deploy this isolated Worker using a dedicated `CLOUDFLARE_V3_TEST_API_TOKEN` GitHub Actions secret. It runs only on updates to the PR from this same repository and exact staging branch. The account ID may come from the existing `CLOUDFLARE_ACCOUNT_ID` secret or, if absent, from the token when it sees exactly one account. The dedicated token must allow D1 read and write, R2 storage read and write, and Worker script deployment on the ALYZIA account. The job first confirms the production D1 and R2 resources belong to the target account, then creates distinct V3 test resources if absent, applies the V3 migration and deploys the test Worker. If the token lacks D1 or R2 permission, the job fails before touching any production data. The V3 admin secret is derived for the test Worker within the runner and is never written to the repository or printed in the job logs. Keep the production `CLOUDFLARE_API_TOKEN` unchanged.

Set `CLOUDFLARE_V3_TEST_API_TOKEN` under this repository's **Settings > Secrets and variables > Actions > Repository secrets**. Environment or organization secrets with restricted access are not necessarily available to this PR workflow. A newly queued PR run will read the repository secret; never paste its value into issues, comments or job logs.
